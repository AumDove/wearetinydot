...
function meg_setup() {
	 ...
	 * If you're building a theme based on Meg, use a find and replace
	 * to change 'meg' to the name of your theme in all the template files.
	 */
	load_theme_textdomain( 'meg', get_template_directory() . '/languages' );
	...
	add_theme_support( 'post-thumbnails' );
        ...
        add_image_size( 'meg-small-thumb', 300, 150, true);
	...
	add_theme_support( 'custom-background', apply_filters( 'meg_custom_background_args', array(
...
add_action( 'after_setup_theme', 'meg_setup' );
 ...
 */
function meg_content_width() {
	$GLOBALS['content_width'] = apply_filters( 'meg_content_width', 640 );
}
add_action( 'after_setup_theme', 'meg_content_width', 0 );
 ...
 */
function meg_widgets_init() {
	register_sidebar( array(
		'name'          => esc_html__( 'Widget Area', 'meg' ),
		'id'            => 'sidebar-1',
		'description'   => esc_html__( 'Add widgets here.', 'meg' ),
...
add_action( 'widgets_init', 'meg_widgets_init' );

/**
 * Enqueue scripts and styles.
 */
function meg_scripts() {
	wp_enqueue_style( 'meg-style', get_stylesheet_uri() );
	...
	wp_enqueue_script( 'meg-navigation', get_template_directory_uri() . '/js/function.js', array( 'jquery' ), '20151215', true );
		...
		'expand'   => '<span class="screen-reader-text">' . __( 'expand child menu', 'meg' ) . '</span>',
		'collapse' => '<span class="screen-reader-text">' . __( 'collapse child menu', 'meg' ) . '</span>',
	) );

	wp_enqueue_script( 'meg-skip-link-focus-fix', get_template_directory_uri() . '/js/skip-link-focus-fix.js', array(), '20151215', true );
